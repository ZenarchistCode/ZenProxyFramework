...
I learned the method for identifying proxy objects through studying their code, but my framework mod is written from scratch with the intention of creating a codebase for modders to add their own custom actions to in-game proxy objects.
...
The mod has a debug mode in which you can press "F" on various objects to get their proxy name. Then to add an action to it, just follow the template code provided.
