class ZenHouseProxyPlugin extends PluginBase
{
	...
	override void OnInit()
	{
		...
		{
			m_BeginTimestamp = g_Game.GetTime();
	...
	override void OnUpdate(float delta_time)
	{
		...
		if (!m_HasWorldInitializedServer)
		{
			...
			if (House.LAST_ZENPROXY_HOUSE_SPAWN_TIMESTAMP > 0 && g_Game.GetTime() - House.LAST_ZENPROXY_HOUSE_SPAWN_TIMESTAMP >= 30000)
			{
				FinalizeInit();
	...
	void FinalizeInit()
	{
		...
		Print("[ZenHouseProxyPlugin] Init complete: Total unique proxy'd buildings=" + GetZenProxyFrameworkConfig().HouseProxyData.Count() + " (" + proxyCount + " proxy locations in memory) Time = " + ((g_Game.GetTime() - m_BeginTimestamp) / 1000) + " secs.");
	...
	int AddCustomProxyOverrides()
	{
		...
		{
			AddHouseProxyData(proxyOverride.HouseConfigType, proxyOverride.ProxyName, proxyOverride.ProxyLocation.ToVector());
			proxyCount++;
		}
