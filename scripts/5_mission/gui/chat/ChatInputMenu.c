        ...
        if (GetZenProxyFrameworkConfig().DebugOn)
        {
            if (finished)
            {
                ...
                if (nearestBuilding)
                {
                    ...
                    g_Game.GetMission().OnEvent(ChatMessageEventTypeID, new ChatMessageEventParams(CCDirect, "", "Pos for " + nearestBuilding.GetType() + ": " + mapPosOffset, ""));
                    g_Game.CopyToClipboard(mapPosOffset.ToString(false));
                }
                else
                {
                    g_Game.GetMission().OnEvent(ChatMessageEventTypeID, new ChatMessageEventParams(CCDirect, "", "NO BUILDING DETECTED!", ""));
                }
